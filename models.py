...
class Conv_block4NetD(nn.Module):
    def __init__(self, in_channels, out_channels, stride):
        ...
        self.conv = nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=4, padding=1, stride=stride,
                              bias=False)
...
class netD(nn.Module):
    def __init__(self, input_channel=3, mid_channel=64):
        ...
        self.Conv_blockIn = Conv_block4NetD(self.input_channel,
                                            self.mid_channel // 2,2)  # output size = self.mid_channel/2 x 64 x 64
        self.Conv_block1 = Conv_block4NetD(self.mid_channel // 2,
                                           self.mid_channel,2)  # output size = self.mid_channel x 32 x 32
        self.Conv_block2 = Conv_block4NetD(self.mid_channel * 1,
                                           self.mid_channel * 2,2)  # output size = self.mid_channel * 2 x 16 x 16
        self.Conv_block3 = Conv_block4NetD(self.mid_channel * 2,
                                           self.mid_channel * 4,1)  # output size = self.mid_channel * 4 x 8 x 8
        self.Conv_block4 = Conv_block4NetD(self.mid_channel * 4,
                                           self.mid_channel * 8,1)  # output size = self.mid_channel * 8 x 4 x 4
...
class netSR(nn.Module):
    def __init__(self, input_channel=3, mid_channel=32):
        self.input_channel = input_channel
        self.mid_channel = mid_channel
        ...
        self.Conv_block1 = Conv_block4NetSR(self.mid_channel, self.mid_channel*2)
        self.Conv_block2 = Conv_block4NetSR(self.mid_channel*2, self.mid_channel*4)
        self.Conv_block3 = Conv_block4NetSR(self.mid_channel*4, self.mid_channel*4)
        self.Conv_block4 = Conv_block4NetSR(self.mid_channel*4, self.mid_channel*4)
        self.Conv_block5 = Conv_block4NetSR(self.mid_channel*4, self.mid_channel*2)
        self.Convup = nn.Sequential(nn.ConvTranspose2d(mid_channel*2, mid_channel, kernel_size=4, stride=2, padding=1),
                                    nn.LeakyReLU(0.2))
