...
class ZSSR:
    ...
    def __init__(self, input_img, conf=Config(), ground_truth=None, kernels=None):
        ...
        self.netD = netD(3, mid_channel=64).cuda()
    ...
    def forward_backward_pass(self, lr_son, hr_father,criterion,optimizer,criterion_G, criterion_vgg , criterion_D, optimizer_D):
        ...
        loss_G = criterion_G(self.netD(train_output), true_labels)
        loss_percep = criterion_vgg(self.vgg(train_output), self.vgg(hr_father)) 
        loss_G_total = loss + loss_G + loss_percep
    ...
    def train(self):
        ...
        optimizer_D = torch.optim.Adam(self.netD.parameters(), lr=self.learning_rate)
        ...
        for self.iter in range(self.conf.max_iters):
            ...
            self.train_output = self.forward_backward_pass(self.lr_son, self.hr_father,criterion,optimizer ,criterion_G, criterion_vgg,criterion_D, optimizer_D)

            # Display info and save weights
            if not self.iter % self.conf.display_every:
                print('sf:', self.sf*self.base_sf, ', iteration: ', self.iter, ', pixel loss: ', self.loss[self.iter].item(), "GAN loss: ", self.GANloss.item(), "percep loss ", self.loss_percep.item())
    ...
    def father_to_son(self, hr_father):
        ...
        return np.clip(lr_son + np.random.randn(*lr_son.shape) * 0.1, 0, 1)
